...
public class LosthorizonModVillagerProfessions {
	...
	public static final DeferredHolder<VillagerProfession, VillagerProfession> NECROMANCER = registerProfession("necromancer", () -> LosthorizonModBlocks.ONYX_ORE.get(),
	...
	private static DeferredHolder<VillagerProfession, VillagerProfession> registerProfession(String name, Supplier<Block> block, Supplier<SoundEvent> soundEvent) {
		...
		return PROFESSIONS.register(name, () -> {
			...
			return new VillagerProfession(LosthorizonMod.MODID + ":" + name, poiPredicate, poiPredicate, ImmutableSet.of(), ImmutableSet.of(), soundEvent.get());
		});
	}
		...
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			for (Map.Entry<String, ProfessionPoiType> entry : POI_TYPES.entrySet()) {
				...
				if (existingCheck.isPresent()) {
					LosthorizonMod.LOGGER.error("Skipping villager profession " + name + " that uses POI block " + block + " that is already in use by " + existingCheck);
					continue;
				}
				PoiType poiType = new PoiType(ImmutableSet.copyOf(block.getStateDefinition().getPossibleStates()), 1, 1);
				registerHelper.register(ResourceLocation.fromNamespaceAndPath("losthorizon", name), poiType);
